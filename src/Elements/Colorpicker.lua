...
function Element:New(Idx, Config)
	...
	local function CreateColorDialog()
		...
		Creator.AddSignal(RedInput.Input.FocusLost, function(Enter)
			if Enter then
				local num = tonumber(RedInput.Input.Text)
				if num and num >= 0 and num <= 255 then
					local CurrentColor = GetRGB()
					local Success, Result = pcall(Color3.fromRGB, num, CurrentColor["G"], CurrentColor["B"])
					if Success and typeof(Result) == "Color3" then
						Hue, Sat, Vib = Color3.toHSV(Result)
					end
				end
		...
		Creator.AddSignal(GreenInput.Input.FocusLost, function(Enter)
			if Enter then
				local num = tonumber(GreenInput.Input.Text)
				if num and num >= 0 and num <= 255 then
					local CurrentColor = GetRGB()
					local Success, Result = pcall(Color3.fromRGB, CurrentColor["R"], num, CurrentColor["B"])
					if Success and typeof(Result) == "Color3" then
						Hue, Sat, Vib = Color3.toHSV(Result)
					end
				end
		...
		Creator.AddSignal(BlueInput.Input.FocusLost, function(Enter)
			if Enter then
				local num = tonumber(BlueInput.Input.Text)
				if num and num >= 0 and num <= 255 then
					local CurrentColor = GetRGB()
					local Success, Result = pcall(Color3.fromRGB, CurrentColor["R"], CurrentColor["G"], num)
					if Success and typeof(Result) == "Color3" then
						Hue, Sat, Vib = Color3.toHSV(Result)
					end
				end
